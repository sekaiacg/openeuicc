        ...
        vendorInfo?.let {
            ...
            vendorInfo.region?.let {
                val region = getString(
                    when (it) {
                        0 -> R.string.euicc_info_region_global
                        1 -> R.string.euicc_info_region_china
                        else -> R.string.euicc_info_region_global
                    }
                )
                val languages = getString(
                    when (it) {
                        0 -> R.string.euicc_info_region_global_languages
                        1 -> R.string.euicc_info_region_china_languages
                        else -> R.string.euicc_info_region_global_languages
                    }
                )
                val regionAndLanguagesStr = "0$it(${region})→${languages}"
                add(Item(R.string.euicc_info_region_and_languages, regionAndLanguagesStr))
            }
            showEum = false
        }
