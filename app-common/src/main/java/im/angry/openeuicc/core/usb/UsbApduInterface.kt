        ...
        try {
            ...
            if (isSuccessResponse(resp)) {
                ...
                if (isSuccessResponse(selectAidResp)) {
                    ...
                    if (isSuccessResponse(ret)) {
                        estkInfo?.blVer = String(ret, 0, ret.size - 2, Charsets.UTF_8)
                    }
                    ret = transmitApduByChannel(estkGetFwVerAPDU, cid.toByte())
                    if (isSuccessResponse(ret)) {
                        estkInfo?.fwVer = String(ret, 0, ret.size - 2, Charsets.UTF_8)
                    }
                    ret = transmitApduByChannel(estkGetSkuAPDU, cid.toByte())
                    if (isSuccessResponse(ret)) {
                        estkInfo?.sku = String(ret, 0, ret.size - 2, Charsets.UTF_8)
                    }
                }
            }
        } catch (_: Exception) {
