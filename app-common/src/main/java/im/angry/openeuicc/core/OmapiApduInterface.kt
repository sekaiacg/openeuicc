...
class OmapiApduInterface(
    ...
    private fun tryGetEstkInfo() {
        ...
        try {
            ...
            transmit(estkGetBlVerAPDU, channel).let {
                estkInfo?.blVer = String(it, 0, it.size - 2, Charsets.UTF_8)
            }
            transmit(estkGetFwVerAPDU, channel).let {
                estkInfo?.fwVer = String(it, 0, it.size - 2, Charsets.UTF_8)
            }
            transmit(estkGetSkuAPDU, channel).let {
                estkInfo?.sku = String(it, 0, it.size - 2, Charsets.UTF_8)
            }
        } catch (_: Exception) {
        } finally {
            channel?.close()
